...
**Codebase:** ~7,250 lines of Swift across 37 source files
**Version:** 0.1.23
...
- **Ghostty** as git submodule (pinned to v1.3.1), xcframework built with `zig build`
...
### Models (Sources/Models/) - 1,645 lines total
...
### Views (Sources/Views/) - ~3,500 lines total
...
**MarkdownView.swift:**
- `CMARK_OPT_UNSAFE` enabled (allows raw HTML in markdown)
- Mitigated by: JavaScript disabled in WKWebView, navigation policy only allows http/https external links, only renders user's own local README files

- [ ] **LOW - CMARK_OPT_UNSAFE**: Raw HTML in markdown is allowed but mitigated by JS-disabled WKWebView. Consider using `CMARK_OPT_SAFE` if rendering untrusted markdown in the future.
...
- [ ] **LOW** - MD5 used for CSS cache busting in `TerminalContainerView.swift` and website templates. Not a security concern (non-cryptographic use), but could be replaced with SHA-256 for consistency.
...
| TerminalViewTests | Terminal view lifecycle |
...
- [ ] **MEDIUM - No tests for GitOperations**: Git process execution, worktree creation/removal, branch detection, and .env symlinking are untested.
- [ ] **MEDIUM - No tests for Environment.swift**: Async caching, throttling, and refresh logic are untested.
- [ ] **LOW - No integration tests**: No end-to-end tests for project creation -> workstream -> terminal flow.
- [ ] **LOW - No tests for surface lifecycle**: TerminalView event handling and Ghostty surface management are untested.
...
| Critical | 0 | — |
| High | 0 | — |
| Medium | 3 | Surface cache orphaning, GitOperations test gap, Environment.swift test gap |
| Low | 8 | Force unwrap, CMARK_OPT_UNSAFE, MD5 cache busting, AppleScript string interpolation, error suppression, integration test gap, surface lifecycle test gap, large file refactoring |
...
- [ ] Consider `CMARK_OPT_SAFE` if markdown rendering scope expands beyond local READMEs
- [ ] Replace MD5 with SHA-256 for cache busting (non-urgent, cosmetic)
...
#### Testing
- [ ] Add unit tests for `GitOperations` (worktree create/remove, branch detection, .env symlinking)
...
- [ ] Add tests for surface lifecycle management
...
- **Good** test coverage with room for improvement in service-layer testing
