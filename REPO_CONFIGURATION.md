# Adapting Existing Repositories for Dockyard

To get the best experience when using Dockyard with your existing repositories, you need to configure how the IDE sets up, runs, and manages your project's environment.
...
Dockyard looks for a configuration file in the root of your repository to understand how to build and run your project.
...
*Note: Dockyard also supports legacy fallback configuration names like `.emdash.json`, `conductor.json`, or `.superset/config.json` for backwards compatibility.*
...
2.  Dockyard will automatically **symlink** the `.env` file from the base repository into the root of each new workstream directory (if this feature is enabled in your Dockyard settings).
...
When you start your project using the `run` command (e.g., `npm run dev`), Dockyard wraps the process using its `dy-run` launcher.

You **do not** need to manually configure localhost URLs for the built-in browser. The `dy-run` launcher automatically:
...
Because Dockyard uses Git worktrees to parallelize work:
